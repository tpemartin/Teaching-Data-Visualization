...
![xml-structure](./fig/xml-民間消費.png)
<!--![xml-structure](/fig/xml-民間消費.png) "主計處民間消費XML資料結構"-->
...
```{r, eval=FALSE}
xmlAttrs(xmltop[[2]][[1]][[1]][[1]])
xmlGetAttr(xmltop[[2]][[1]][[1]][[1]],"OBS_VALUE")
data.out<-xmlApply(xmltop[[2]][[1]][[1]],xmlAttrs)
library(dplyr)
#data.out %>% unlist() %>%
  matrix(.,length(.)/2,2,byrow=TRUE)
#data.out<-unlist(data.out)
```
